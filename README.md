...
<html lang="pt-BR" class="h-full">
 ...
 <body class="h-full">
        ...
        <form id="group-message-form" onsubmit="sendGroupMessage(event)" class="space-y-2">
         <div id="reply-preview" class="hidden p-2 bg-blue-50 rounded border-l-3 border-blue-500 text-sm">
          <div class="flex justify-between items-start">
           ...
           </div><button type="button" onclick="cancelReply()" class="text-gray-500 hover:text-gray-700">×</button>
          </div>
         </div>
         <div class="flex gap-2 items-end"><textarea id="group-msg-input" class="input flex-1 message-input-field" placeholder="Digite sua mensagem..." required rows="2"></textarea> <label class="btn bg-gray-200 text-gray-700 cursor-pointer"> 📷 <input type="file" id="group-media-input" class="hidden" accept="image/*,video/*,audio/*" onchange="handleMediaUpload(this)"> </label> <button type="submit" class="btn btn-primary">Enviar</button>
  ...
  <script>
    ...
    const MESA_CAMARA = ['Presidente da Camara', 'Vice-Presidente', '1º Secretario', '2º Secretario', 'Lider Maioria', 'Lider Minoria'];
    const MESA_SENADO = ['Presidente do Senado', 'Vice-Presidente', '1º Secretario', '2º Secretario', 'Lider Maioria'];
      ...
      const html = publicParties.length ? publicParties.map(p => {
        ...
        return `<div class="p-2 bg-gray-700 hover:bg-gray-600 rounded cursor-pointer" onclick="viewPublicParty('${p.__backendId}')">
          <div class="flex items-center gap-2">
            ...
            ${isMember ? '<span class="text-xs text-green-400">✓</span>' : ''}
          </div>
        </div>`;
    ...
    function renderPublicPartiesCards() {
      ...
      const html = publicParties.length ? publicParties.map(p => {
        ...
        return `<div class="card hover:shadow-lg transition cursor-pointer" onclick="viewPublicParty('${p.__backendId}')">
          ...
          ${isMember ? 
            `<button onclick="event.stopPropagation(); selectParty('${p.__backendId}')" class="btn bg-green-600 text-white w-full text-sm hover:bg-green-700">✓ Membro - Acessar</button>` :
    ...
    function viewPublicParty(partyId) {
      ...
      showModal(`<div class="p-6 max-h-[85vh] overflow-y-auto">
        ...
        ${isMember ? 
          `<button onclick="closeModal(); selectParty('${party.__backendId}')" class="btn bg-green-600 text-white w-full">✓ Acessar Gabinete</button>` :
    ...
    function renderPublicGroupsCards() {
      ...
      const html = publicGroups.length ? publicGroups.map(g => {
        ...
        return `<div class="card hover:shadow-lg transition cursor-pointer" onclick="viewPublicGroup('${g.groupName}')">
          ...
          ${isMember ? 
            `<button onclick="event.stopPropagation(); selectGroup('${g.groupName}')" class="btn bg-green-600 text-white w-full text-sm hover:bg-green-700">✓ Membro - Acessar</button>` :
    ...
    function viewPublicGroup(groupName) {
      ...
      showModal(`<div class="p-6 max-h-[85vh] overflow-y-auto">
        ...
        ${isMember ? 
          `<button onclick="closeModal(); selectGroup('${groupMsg.groupName}')" class="btn bg-green-600 text-white w-full">✓ Acessar Grupo</button>` :
      ...
      showModal(`<div class="p-6">
        ...
        <div class="space-y-3">
          ...
          ${p.pdfData ? `<div class="p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p class="font-bold text-sm mb-2 text-blue-700">📄 Documento Completo</p>
    ...
    function showModal(html) {
      document.getElementById('modal-container').innerHTML = `<div class="modal" onclick="if(event.target === this) closeModal()">
        <div class="modal-content">${html}<button onclick="closeModal()" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 text-2xl">×</button></div>
      </div>`;
    }
